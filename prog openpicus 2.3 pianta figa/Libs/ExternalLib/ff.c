...
)
{
	...
	vol = px[0] - '0';					/* Is there a drive number? */
	if (vol <= 9 && px[1] == ':') {		/* Found a drive number, get and strip it */
		px += 2; *path = px;				/* Return pointer to the path name */
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	fs->n_fats = bx = fs->win[BPB_NumFATs];				/* Number of FAT copies */
	if (bx != 1 && bx != 2) return FR_NO_FILESYSTEM;		/* (Must be 1 or 2) */
	fasize *= bx;										/* Number of sectors for FAT area */

	fs->csize = bx = fs->win[BPB_SecPerClus];			/* Number of sectors per cluster */
	if (!bx || (bx & (bx - 1))) return FR_NO_FILESYSTEM;	/* (Must be power of 2) */
